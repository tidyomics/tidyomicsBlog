...
`plyxp` is a high-performance execution engine designed to accelerate data manipulation operations in R, particularly for complex data structures like those used in genomics and single-cell analysis. The name "plyxp" reflects its role as a **ply** (manipulation) **exp** (expression) engine that optimizes the execution of data transformation pipelines.
...
cat("Using existing worktrees for benchmark comparison\n")
cat("Before optimization:", wt_before, "\n")
cat("After optimization:", wt_after, "\n")
