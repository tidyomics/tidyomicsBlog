...
required_packages <- c("devtools", "ggplot2", "dplyr", "SummarizedExperiment", 
                      "airway", "microbenchmark", "rlang", "reactable", "patchwork")
...
#| eval: false
#| code-fold: true
#| code-summary: "Show pre-optimization source"
...
#| eval: false
#| code-fold: true
#| code-summary: "Show post-optimization source"
mutate.SummarizedExperiment <- function(.data, ...) {
...
```{r results_interpretation_values}
#| include: false
#| message: false
#| warning: false
speedup_rank <- summary_table |>
  dplyr::arrange(dplyr::desc(speedup))

top3 <- head(speedup_rank, 3)
bottom3 <- tail(speedup_rank, 3) |>
  dplyr::arrange(speedup)

op_speedup <- summary_table |>
  dplyr::mutate(operation = dplyr::case_when(
    grepl("^filter", scenario) ~ "filter",
    grepl("^select", scenario) ~ "select",
    grepl("^distinct", scenario) ~ "distinct",
  ...
  dplyr::summarise(median_speedup = median(speedup), .groups = "drop") |>
  dplyr::arrange(dplyr::desc(median_speedup))

speedup_min <- round(min(summary_table$speedup), 2)
speedup_max <- round(max(summary_table$speedup), 2)
top3_text <- paste0(top3$scenario, " (", round(top3$speedup, 2), "x)", collapse = ", ")
bottom3_text <- paste0(bottom3$scenario, " (", round(bottom3$speedup, 2), "x)", collapse = ", ")
op_speedup_text <- paste0(op_speedup$operation, " (", round(op_speedup$median_speedup, 2), "x)", collapse = ", ")
```

Across all scenarios, speedup ranges from **`r speedup_min`x** to **`r speedup_max`x**.

Operations with the strongest gains are: **`r top3_text`**.

Lower-gain outliers are: **`r bottom3_text`**.

By operation family, median speedup is: **`r op_speedup_text`**.
